...
namespace MSRP
{
    ...
    public class TransactionManager : Observer
    {
            ...
            if (transaction.TransactionType == TransactionType.SEND)
            {
                ...
                if (message != null && message.IsComplete)
                {
                    ...
                    try
                    {
                        ...
                        long callCount = message.Counter.Count;

                        message.ReportMechanism.TriggerSuccessReport(message, transaction, message.LastCallReportCount, callCount);
        ...
        {
            if (messageToSend == null || messageToSend.Direction != Direction.OUT) { return; }

            Transaction newTransaction = new Transaction((OutgoingMessage)messageToSend, this);
